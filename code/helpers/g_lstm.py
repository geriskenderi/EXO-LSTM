...
class Model(nn.Module):
    ...
    def forward(self, input, future=0, y=None):
        ...
        h_t2 = torch.zeros(input.size(0), self.hidden_size, dtype=torch.float32)
        c_t2 = torch.zeros(input.size(0), self.hidden_size, dtype=torch.float32)


        for i, input_t in enumerate(input.chunk(input.size(1), dim=1)):
            h_t, c_t = self.lstm(input_t.squeeze(1), (h_t, c_t))
            h_t2, c_t2 = self.lstm2(h_t.squeeze(1), (h_t2, c_t2))         
